...
void gyroUpdate(void)
{
    ...
    if (gyroLpfCutFreq) {
        if (!gyroFilterStateIsSet) initGyroFilterCoefficients();

        if (gyroFilterStateIsSet) {
            for (axis = 0; axis < 3; axis++) gyroADC[axis] = lrintf(applyBiQuadFilter((float) gyroADC[axis], &gyroFilterState[axis]));
        }
    }
