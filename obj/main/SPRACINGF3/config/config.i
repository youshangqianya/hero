...
typedef enum {
    NRF24RX_V202_250K = 0,
    NRF24RC_PROVIDER_MAX = NRF24RX_V202_250K
} NRF24RXType;
...
# 89 "./src/main/config/config.c"
#define FLASH_PAGE_SIZE ((uint16_t)0x800)
# 112 "./src/main/config/config.c"
#define FLASH_PAGE_COUNT ((FLASH_SIZE * 0x400) / FLASH_PAGE_SIZE)
# 126 "./src/main/config/config.c"
...
# 403 "./src/main/config/config.c"
    ...
    masterConfig.failsafeConfig.failsafe_procedure = 0;
...
{
    const master_t *temp = (const master_t *) (0x08000000 + (uint32_t)((((uint16_t)0x800) * ((256 * 0x400) / ((uint16_t)0x800))) - 0x1000));
    uint8_t checksum = 0;
...
void validateAndFixConfig(void)
{
    ...
    if (featureConfigured(FEATURE_SOFTSERIAL) && (
            ...
            || (((TIM_TypeDef *) ((((uint32_t)0x40000000) + 0x00010000) + 0x00002C00)) == ((TIM_TypeDef *) (((uint32_t)0x40000000) + 0x00000400)))
...
void readEEPROM(void)
{
    ...
    memcpy(&masterConfig, (char *) (0x08000000 + (uint32_t)((((uint16_t)0x800) * ((256 * 0x400) / ((uint16_t)0x800))) - 0x1000)), sizeof(master_t));
...
void writeEEPROM(void)
{
    ...
    while (attemptsRemaining--) {
        ...
        for (wordOffset = 0; wordOffset < sizeof(master_t); wordOffset += 4) {
            if (wordOffset % ((uint16_t)0x800) == 0) {
                status = FLASH_ErasePage((0x08000000 + (uint32_t)((((uint16_t)0x800) * ((256 * 0x400) / ((uint16_t)0x800))) - 0x1000)) + wordOffset);
                if (status != FLASH_COMPLETE) {
                    break;
                }
            }

            status = FLASH_ProgramWord((0x08000000 + (uint32_t)((((uint16_t)0x800) * ((256 * 0x400) / ((uint16_t)0x800))) - 0x1000)) + wordOffset,
