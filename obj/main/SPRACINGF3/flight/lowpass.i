...
# 19 "./src/main/flight/lowpass.c" 2
...
# 20 "./src/main/flight/lowpass.c" 2
...
# 21 "./src/main/flight/lowpass.c" 2
...
# 22 "./src/main/flight/lowpass.c" 2
...
# 24 "./src/main/flight/lowpass.c" 2
...
# 25 "./src/main/flight/lowpass.c" 2
...
void generateLowpassCoeffs2(int16_t freq, lowpass_t *filter)
{
    ...
    float omega = tanf((float)3.14159265358979323846*freqf/2.0f);
    ...
    for (i = 0; i < 3; i++) {
        filter->a[i] = (filter->af[i] * fixedScaler < 0 ? (filter->af[i] * fixedScaler - 0.5f) : (filter->af[i] * fixedScaler + 0.5f));
        filter->b[i] = (filter->bf[i] * fixedScaler < 0 ? (filter->bf[i] * fixedScaler - 0.5f) : (filter->bf[i] * fixedScaler + 0.5f));
...
int32_t lowpassFixed(lowpass_t *filter, int32_t in, int16_t freq)
{
    ...
    if (!filter->init) {
        ...
        for (coefIdx = 0; coefIdx < 3; coefIdx++) {
    ...
    for (coefIdx = 3 -1; coefIdx > 0; coefIdx--) {
    ...
    for (coefIdx = 1; coefIdx < 3; coefIdx++) {
