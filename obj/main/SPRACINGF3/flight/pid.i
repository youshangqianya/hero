...
# 6 "/usr/include/newlib/math.h" 2 3
# 1 "/usr/include/newlib/machine/ieeefp.h" 1 3
# 7 "/usr/include/newlib/math.h" 2 3
# 1 "/usr/include/newlib/_ansi.h" 1 3
...
# 108 "/usr/include/newlib/math.h" 3
...
# 150 "/usr/include/newlib/math.h" 3
...
# 173 "/usr/include/newlib/math.h" 3
...
# 225 "/usr/include/newlib/math.h" 3
...
# 475 "/usr/include/newlib/math.h" 3
...
# 519 "/usr/include/newlib/math.h" 3
            ...
            rollAndPitchTrims_t *angleTrim, rxConfig_t *rxConfig)
{
    ...
    if ((flightModeFlags & (HORIZON_MODE))) {
        prop = ((((((rcCommand[PITCH]) > 0 ? (rcCommand[PITCH]) : -(rcCommand[PITCH]))) > (((rcCommand[ROLL]) > 0 ? (rcCommand[ROLL]) : -(rcCommand[ROLL]))) ? (((rcCommand[PITCH]) > 0 ? (rcCommand[PITCH]) : -(rcCommand[PITCH]))) : (((rcCommand[ROLL]) > 0 ? (rcCommand[ROLL]) : -(rcCommand[ROLL]))))) < (512) ? (((((rcCommand[PITCH]) > 0 ? (rcCommand[PITCH]) : -(rcCommand[PITCH]))) > (((rcCommand[ROLL]) > 0 ? (rcCommand[ROLL]) : -(rcCommand[ROLL]))) ? (((rcCommand[PITCH]) > 0 ? (rcCommand[PITCH]) : -(rcCommand[PITCH]))) : (((rcCommand[ROLL]) > 0 ? (rcCommand[ROLL]) : -(rcCommand[ROLL]))))) : (512));
    }
    ...
    for (axis = 0; axis < 2; axis++) {
        ...
        if (((gyroADC[axis]) > 0 ? (gyroADC[axis]) : -(gyroADC[axis])) > (640 * 4)) {
            errorGyroI[axis] = 0;
        }

        ITerm = (errorGyroI[axis] >> 7) * pidProfile->I8[axis] >> 6;
        ...
        if ((flightModeFlags & (ANGLE_MODE)) || (flightModeFlags & (HORIZON_MODE))) {
            ...
            PTermACC = ((int32_t)errorAngle * pidProfile->P8[PIDLEVEL]) >> 7;

            int16_t limit = pidProfile->D8[PIDLEVEL] * 5;
            PTermACC = constrain(PTermACC, -limit, +limit);

            ITermACC = ((int32_t)errorAngleI[axis] * pidProfile->I8[PIDLEVEL]) >> 12;

            ITerm = ITermACC + ((ITerm - ITermACC) * prop >> 9);
            PTerm = PTermACC + ((PTerm - PTermACC) * prop >> 9);
        }

        PTerm -= ((int32_t)gyroError * dynP8[axis]) >> 6;
        ...
        if (pidProfile->dterm_cut_hz) {
            DTerm = lrintf(applyBiQuadFilter((float) DTerm, &deltaFilterState[axis])) * 3;
        }
    ...
    if (((rc) > 0 ? (rc) : -(rc)) > 50) errorGyroI[FD_YAW] = 0;

    PTerm = (int32_t)error * pidProfile->P8[FD_YAW] >> 6;


    if(motorCount >= 4 && pidProfile->yaw_p_limit < 500) {
        PTerm = constrain(PTerm, -pidProfile->yaw_p_limit, pidProfile->yaw_p_limit);
    }

    ITerm = constrain((int16_t)(errorGyroI[FD_YAW] >> 13), -256, +256);
        ...
        rollAndPitchTrims_t *angleTrim, rxConfig_t *rxConfig)
{
    ...
    for (axis = 0; axis < 3; axis++) {
        ...
        errorGyroI[axis] = constrain(errorGyroI[axis], (int32_t) - 256 << 13, (int32_t) + 256 << 13);
        ...
        if (pidProfile->dterm_cut_hz) {
            deltaSum = lrintf(applyBiQuadFilter((float) deltaSum, &deltaFilterState[axis])) * 3;
        }
